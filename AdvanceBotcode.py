...
async def delete_work(ctx, name: str = None):
	if name is None:
		await ctx.send("Укажите название профессию!")
	else:
...
async def casino(ctx, amount: int = False):
	if amount is None:
		...
		await ctx.send("Сумма не может быть равна 0!")
...
async def Cube(ctx, number: int = None):
	if number is None:
		...
		if number == rand_cube:
			await ctx.send(f"{ctx.author.mention}, вы угадали! Ваш приз 10000 пуллов")
